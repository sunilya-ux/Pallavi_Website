...
import { CheckCircle, Loader, Copy, Check, RotateCcw, Sparkles, Download, X, DollarSign, FileSpreadsheet } from 'lucide-react';
...
export default function BigMoneyContentGenerator({ clientId }: BigMoneyContentGeneratorProps) {
  ...
  const handleDownloadExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, '7-Day Content Plan');
      XLSX.writeFile(workbook, 'big-money-content-plan.xlsx');

      setExportMessage('Excel file downloaded successfully!');
      setTimeout(() => setExportMessage(''), 3000);
    } catch (error) {
      console.error('Excel export error:', error);
      setExportMessage('Failed to generate Excel file. Please try again.');
      setTimeout(() => setExportMessage(''), 5000);
    }
  };
          ...
          <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl shadow-lg border border-amber-200 p-5 sm:p-6">
            ...
            <div className="space-y-4">
              <div className="flex flex-wrap gap-3">
                ...
                <button onClick={handleDownloadExcel} className="flex items-center gap-2 px-6 py-3 bg-white hover:bg-slate-50 text-slate-700 rounded-lg transition-all shadow-sm hover:shadow-md border border-slate-200 font-medium">
                  <FileSpreadsheet className="w-5 h-5" /><span>Download Excel</span>
                </button>
